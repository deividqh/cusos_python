...
class listTOdict_byTcld():
    ...
    def byTcld(listaStrKeys, listaDef=None, msgIntro='Intro', permiteNulo=False, esCapital=False):
        ...
        if listaDef:       
            ...
            # _________________
            # CREA EL DICCIONARIO de retorno Generando un patron(cualquier frase) para ejecutar una funcion(__introByTcld)
            dictRetorno = {
                strKey:re.sub(
                    pattern=patron, 
    ...
    def __introByTcld(strValor, listaDefinicion, options=None):
        ...
        while True:
            retorno = input(f'{msgIntro} {valorLista} - ( {listaDefinicion[TIPO].__name__} - {strnulo} )..... ')
            try:
