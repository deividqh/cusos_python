...
# Definicion de una Enumeracion(para no tener que usar index). 
    ...
    # 3-Uso: dP.TITULO.values(para el nombre de la pelicula) o tU.US.values(para el nombre de usuario)
    #           
...
def entraUsuario(*usuario):
    ...
    while(True):
        ...
        if i==None: 
            ...
            #     if g.upper()==introGen.upper():
            #         bMatch=True
            #         print (f'Nombre: {n} \tGenero: {g} \tAño: {d}')
                
            # print(f"Genero {introGen} NO Encontrado :(\n"+'*'*AS) if bMatch==False else print ("*"*AS)    
            ...
            # Creo una lista(menuXaFav) con Los Nombres de las Peliculas para crear el menu.            
            # menuXaFav=[]
            # for n in tuplePeliculas:
            #     # menuXaFav.append(n[dP.TITULO.value])                
            #     menuXaFav.append(f"Titulo: {n[dP.TITULO.value]} Genero:{n[dP.GENERO.value]} Año: {n[dP.ANNO.value]}")                
            ...
            # print (menuXaFav)
            # fav=MenuLista(menuXaFav, "Elige pelicula Favorita") 
            fav=MenuLista(listEnUnString, "Elige pelicula Favorita") 
            # del menuXaFav           
            if fav==None: continue

            # tengo la pelicula!! tuplePeliculas[fav-1] (fav-1 pq en el menu añado la opcion salir como 0)
            # print(f"num: {fav}, pelicula: {tuplePeliculas[fav-1][dP.TITULO.value]}")
...
# Uso la funcion format de str. 
# formatoStrFila = "{:<" + str(maxTit) + "}" ==> significa:  
#       {:} = todo se pone despues de esto.  
#       < = alineado a la izquierda 
#       str(numero) = numero de espacios que se pueden poner. 
#       Luego {:<15} reserva 15 espacios alineados a la izquierda. 
#       Luego "{:<" + str(maxTit) + "} {:<" + str(maxGen) + "} {:<" + str(maxAnn) + "}"  = Reserva maxTit , maxGen y maxAnn espacios 
...
def verPeliculas():
    ...
    # Alternativa con formato: {:<15} es alinear a la izquierda y dejar 15 espacios para esa columna
...
# ejemplo:  getReListTuplePelicula(True, False, false) => titulo=True, Genero=False, Anno=false
#           getReListTuplePelicula(True, None, False) => titulo=True, Genero=False, Anno=false
#           getReListTuplePelicula(false, "", True) => Error
#           
#           getReListTuplePelicula(tuplePelis, '101') = True, False, True
# Para un numero indeterminado de argumentos en la pelicula
...
# Lo voy a usar a la hora de mostrar un listado de peliculas
# Esto es muy particular para mostrar los datos en la consola, para web o app hay que cambiarlo.
# getMaxLenPeliculas(tuplePeliculas, 0])
...
# Devuelve: un list con un solo string por fila => listRetorno(stringResultado)
def getListString(listadoPeliculas, bFormato=True):    
    ...
    if bFormato==False:
        ...
        # Generador de formatString:
            # formatoStrFila = "{:<" + str(25) + "} {:<" + str(10) + "}"
            # formatoStrFila = "{:<15} {:<12} {:<5}"
        formatoStrFila=''
        ...
        # Validacion(Comentar o Borrar)
        # print(formatoStrFila)        
...
# -------------------------------------------------------------------------->
# Ahora me falta integrar esto en la generacion de menus 
# y en la funcionalidad del programa(funciona), pero esto es la pera pq se puede crecer muy facilmente.
# Quiero investigar y practicar mas en el dinamismo y la compresion de listas. 

# listadoPeliculas=getReListTuplePelicula(tuplePeliculas,  True, False, True)
# listEnUnString=getListString(tuplePeliculas, bFormato=True)
# imprimeTuplePeliculas(tuplePeliculas, False, True, True)
# --------------------------------------------------------------------------]
