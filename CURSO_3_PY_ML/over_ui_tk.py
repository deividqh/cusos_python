...
def main_row_form():
    ...
    lbl_nom = tk.Label(F1.to_row(1), text='Nombre: ')
    txt_nom = tk.Entry(F1.to_row(1))    
    lbl_ape = tk.Label(F1.to_row(2), text='Apellido: ')
    txt_ape = tk.Entry(F1.to_row(2))    
    btn_add = tk.Button(F1.to_row(3), text="Añadir")
    btn_del = tk.Button(F1.to_row(3), text="Borrar")
    btn_upt = tk.Button(F1.to_row(3), text="Actualiza")
    # ■ Posicionamos
    ...
    F1.get_Frame().config(bg="lightgray")
