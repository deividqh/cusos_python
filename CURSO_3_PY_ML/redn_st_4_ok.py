...
* **Tasa de Aprendizaje (Learning Rate):** El tamaño de los "pasos" que da el modelo al corregir sus errores. Puedes usar los botones +/- o escribir directamente el número.
* **Filtros Convolucionales:** Son las "gafas" de la red. Extraen bordes y formas de la imagen.
* **Neuronas Ocultas (Dense):** Es el "cerebro" o capacidad de razonamiento final del modelo.
"""
...
with st.container(border=True):
    ...
    with c1:
        ...
        # NUEVO: Input numérico para LR que permite escribir libremente o usar +/-
...
if iniciar_entrenamiento:
    ...
    for epoch in range(EPOCHS):
        # NUEVO: Cálculo del porcentaje y actualización del texto de estado
        porcentaje_actual = int(((epoch + 1) / EPOCHS) * 100)
        txt_estado.info(f"⏳ **Procesando Época {epoch + 1} de {EPOCHS} ({porcentaje_actual}%)...** Por favor, espera.")
    ...
    txt_estado.success(f"✅ ¡Entrenamiento completado exitosamente! (100%)")
...
# ■■■■■ 6. EXPORTAR INFORME ■■■■■
    st.markdown("---")
    st.markdown("### 📥 Descargar Informe de Entrenamiento")
    
    # 1. Preparamos el texto del informe con los datos actuales
    informe = f"""========================================
...
Nota: Modelo evaluado sobre 9000 imágenes de validación.
========================================"""
